...
def aa_consent(body: AAConsentRequest) -> AAState:
    ...
    # A new_to_idbi session that hasn't finished the four onboarding questions
    # has no synthetic PersonaData injected yet (see app.onboarding) — there is
    # nothing to gate consent against, so record no state and no audit entry.
