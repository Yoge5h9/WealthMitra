    ...
    <div className={cn("flex items-stretch gap-2", className)}>
      {STAGES.map((stage, index) => (
        <div key={stage.status} className="flex flex-1 items-center gap-2">
          <div
            className={cn(
              "flex flex-1 items-center gap-3 rounded-lg border bg-neutral-0 px-3 py-2",
              stage.accent
            )}
          >
            <span className={cn("flex size-7 shrink-0 items-center justify-center rounded-full", stage.dot)}>
              <stage.icon size={14} strokeWidth={2} className="text-neutral-0" aria-hidden="true" />
            </span>
            <div className="min-w-0">
              <p className="text-h4 font-display font-semibold leading-none tabular-nums text-neutral-900">
                {counts[stage.status]}
              </p>
          ...
          {index < STAGES.length - 1 && (
            <ChevronRight size={16} strokeWidth={1.75} className="shrink-0 text-neutral-300" aria-hidden="true" />
          )}
        </div>
      ))}
    </div>
